...
"SELECT users.name, users.age, SUM(pledges.amount) FROM users INNER JOIN pledges ON pledges.user_id==users.id GROUP BY users.name"
end
...
"SELECT users.name, SUM(pledges.amount) FROM users INNER JOIN pledges ON pledges.user_id==users.id GROUP BY users.name ORDER BY SUM(pledges.amount), users.name"
end
...
"SELECT projects.category, SUM(pledges.amount) FROM projects INNER JOIN pledges ON pledges.project_id == projects.id GROUP BY projects.category HAVING projects.category == 'books'""
end
